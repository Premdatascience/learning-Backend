import registration from "../models/UsersModule.js";
// import csvtojson from "csvtojson";
// import multer from "multer";

//add
export const createUesers = async (req, res) => {
  const addUsers = new registration(req.body);
...
export const viewUesersbyID = async (req, res) => {
  try {
    const users = await registration.findOne({ _id: req.params.id });
    res.status(200).json(users);
  } catch (err) {
    res.status(500).json(err);
  }
...
export const DeleteUesers = async (req, res) => {
  try {
    const record = await registration.findByIdAndDelete(req.params.id);
    // await record.remove();
    // res.send({ data: true });
